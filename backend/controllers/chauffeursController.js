...
const createChauffeur = async (req, res) => {
    const { email, firstName, lastName, age, sex, contactNumber, location, description, pricePerDay, image} = req.body;
    try {
        // Check if the required fields are provided
        if (!email || !firstName || !lastName || !age || !contactNumber || !sex || !location || !description || !pricePerDay || !image) {
            throw new Error("All fields must be filled");
        }

        // Create the user
        const chauffeur = await Chauffeur.create({ email, firstName, lastName, age, sex, contactNumber, location, description, pricePerDay, image});
